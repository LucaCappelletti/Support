...
cat $1 | awk -v CUT=$cut -v y=$colonna -v MEDIA=$media '{ Y=$y; if(Y>MEDIA) {Y=MEDIA}; if(Y<CUT) {Y=CUT}; if(min=="") {min=MEDIA}; if(dir=="") {dir=0}; if(dir==0 && Y<min) {min=Y;}; if(dir==0 && Y>min) {print $1,min; min=Y; dir=1;}; if(dir==1 && Y>min) {min=Y;}; if(dir==1 && Y<min) {min=Y; dir=0;};} END {}'